#!/usr/bin/env python3
"""
Test Fine-tuned Financial Text-to-SQL Model
Demonstrates the model's ability to convert natural language to SQL
"""
...
def load_finetuned_model():
    ...
    try:
        ...
        model_path = "./fixed_financial_model"
...
def generate_sql_query(model, tokenizer, query: str) -> str:
    """Generate SQL from natural language query using fine-tuned model"""
    try:
        # Create prompt
...
def main():
    logger.info("=" * 60)
    logger.info("Testing Fine-tuned Financial Text-to-SQL Model")
    logger.info("=" * 60)
    ...
    try:
        ...
        if accuracy > 0:
            ...
            logger.info("The model can now convert financial queries to SQL!")
        else:
            logger.info("⚠️ Model needs more training or different approach")
